...
export async function POST(req) {
    try {
        ...
        if (payload.add) {
            ...
            let results = await JobCategorySchema.findOne({ name: payload.name, status: { $in: ['0', '1'] } })
            if (!results) {
                let obj = {
                    name: payload.name,
                    slug: payload.slug,
                ...
                const resultsave = new JobCategorySchema(obj);
            ...
            if (result) {
                ...
                success = false
                message = "Job Category found not found"
            }
        }
        else if (payload.edit) {
            let results = await JobCategorySchema.findOne({ _id: payload.id })
            if (results) {
                let namefilterresult = await JobCategorySchema.findOne({ name: payload.name })
                if (namefilterresult) {
                    if (namefilterresult.name == payload.name && namefilterresult._id == payload.id) {
                        ...
                        result = await JobCategorySchema.findOneAndUpdate({ _id: payload.id }, data)
                        if (result) {
                            responsestatus = StatusCodes.SUCCESS
                            success = true
                            message = "Job Category update successfully"
                        } else {
                            responsestatus = StatusCodes.INTERNAL_SERVER_ERROR
                            success = false
                            message = "Internal Server Error"
            ...
            result = await JobCategorySchema.findOneAndUpdate({ _id: payload.id }, { status: payload.status })
            if (result) {
                if (payload.status == "1") {
                    message = result.name + " Job Category inactive successfully"
                } else if (payload.status == "2") {
                    message = result.name + " Job Category delete successfully"
                } else {
                    message = result.name + " Job Category active successfully"
                }
                responsestatus = StatusCodes.SUCCESS
                success = true
            } else {
